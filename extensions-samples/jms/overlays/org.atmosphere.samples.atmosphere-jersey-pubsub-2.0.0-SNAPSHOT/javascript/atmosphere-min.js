...
if(M.reconnectInterval>0){M.id=setTimeout(function(){G(true)
},M.reconnectInterval)
}else{G(true)
}U.responseBody="";
U.messages=[]
...
if(M.reconnectInterval>0){M.id=setTimeout(function(){U.responseBody="";
U.messages=[];
ah(true)
},M.reconnectInterval)
}else{U.responseBody="";
...
if(ay>0){M.id=setTimeout(function(){r()
},ay)
}else{r()
...
clearTimeout(az.id);
if(aA>0){az.id=setTimeout(function(){q(az)
},aA)
}else{q(az)
...
if(k++<ay.maxReconnectOnClose){if(ay.reconnectInterval>0){ay.id=setTimeout(function(){L("re-connecting",az.transport,az);
N(ay)
},ay.reconnectInterval)
}else{L("re-connecting",az.transport,az);
N(ay)
}}else{ad(0,"maxReconnectOnClose reached")
}}};
ax.onload=function(){};
...
if(az.reconnectInterval>0){az.id=setTimeout(function(){at(az)
...
L("re-connecting",az.transport,az);
if(k++<az.maxReconnectOnClose){if(az.reconnectInterval>0){az.id=setTimeout(function(){at(az)
},az.reconnectInterval)
}else{at(az)
}}else{ad(0,"maxReconnectOnClose reached")
}aB.execCommand("Stop");
