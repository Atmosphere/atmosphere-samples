...
i.onmessage=function(ay){clearTimeout(G.id);
G.id=setTimeout(function(){ab(true);
v();
ae()
},G.timeout);
if(ay.origin!=window.location.protocol+"//"+window.location.host){jQuery.atmosphere.log(G.logLevel,["Origin was not "+window.location.protocol+"//"+window.location.host]);
return
...
G.id=setTimeout(function(){av=true;
ab(true);
ae();
v()
},G.timeout);
...
O.messages=[];
u()
}function q(ax,aw,at){if(!n(G,ax)){return true
...
if(at.reconnect&&f++<at.maxReconnectOnClose){F("re-connecting",av.transport,av);
J(au,at,true)
...
at.id=setTimeout(function(){O.errorHandled=true;
ab(true);
v();
ae()
},at.timeout);
...
at.onerror=function(){if(au.transport!="polling"){ae();
if(f++<au.maxReconnectOnClose){au.id=setTimeout(function(){F("re-connecting",av.transport,av);
H(au)
...
aD=aD.substring(az);
az+=aD.length;
if(aA!="polling"){au.id=setTimeout(function(){f=au.maxReconnectOnClose;
ab(true);
v();
ae()
},au.timeout);
...
}if(!aC){x(O.responseBody,"messageReceived",200,aA)
}if(!au.executeCallbackBeforeReconnect){ax()
}}};
...
break
}}function ab(at){if(O.state!="closed"){O.state="closed";
O.responseBody="";
O.messages=[];
O.status=!at?501:200;
u()
