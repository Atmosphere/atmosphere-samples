...
}if(aw.trackMessageLength){ax=at.partialMessage+ax.replace(/(\r\n|\n|\r)/gm,"").replace(/^\s+|\s+$/g,"");
var av=[];
...
if(au>0){H.id=setTimeout(function(){m()
},au)
}else{m()
...
var ay=false;
var aD=false;
if(at.transport==="streaming"&&at.readyState>2&&au.readyState===4){af();
aw();
return
...
if(at.transport!=="polling"){if((!at.enableProtocol||!av.firstMessage)&&au.readyState===2){ai(at)
}var ax=0;
...
if(aw.connectTimeout>0){aw.id=setTimeout(function(){if(aw.requestCount===0){af();
...
clearTimeout(av.id);
if(aw>0){av.id=setTimeout(function(){l(av)
},aw)
}else{l(av)
...
if(f++<au.maxReconnectOnClose){if(au.reconnectInterval>0){au.id=setTimeout(function(){G("re-connecting",av.transport,av);
I(au)
},au.reconnectInterval)
}else{G("re-connecting",av.transport,av);
I(au)
}}else{Z(0,"maxReconnectOnClose reached")
}}};
at.onload=function(){};
...
}if(au.connectTimeout>0){au.id=setTimeout(function(){if(au.requestCount===0){af();
...
if(av.reconnectInterval>0){av.id=setTimeout(function(){ao(av)
...
G("re-connecting",av.transport,av);
if(f++<av.maxReconnectOnClose){if(av.reconnectInterval>0){av.id=setTimeout(function(){ao(av)
},av.reconnectInterval)
}else{ao(av)
}}else{Z(0,"maxReconnectOnClose reached")
}ax.execCommand("Stop");
...
if(H.connectTimeout>0){jQuery.ajax({url:at,async:false,timeout:H.connectTimeout})
